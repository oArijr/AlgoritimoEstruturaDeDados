public class ArvoreBST<T extends Comparable<T>> extends ArvoreBinariaAbstract<T> {
    ...
    private void remover(NoArvoreBST<T> noRemover) {
        ...
        if (noRemover.getGrau() == 0) { // é folha
            ...
            NoArvoreBST<T> noSucessor = this.buscarSucessor(noRemover.getInfo());
    ...
    private NoArvoreBST<T> buscarSucessor(T info) {
        NoArvoreBST<T> no = this.buscar(info);
        if(no == null || info.equals(this.encontraMenorElemento())){
            return null;
        }
    ...
    public T encontraMenorElemento() {
        if (this.vazia()) {
            return null;
        }
        if (this.getRaiz().getEsq() == null) {
            return this.getRaiz().getInfo();
        }
        return (((NoArvoreBST<T>) this.getRaiz()).encontraMenorElemento()).getInfo();
    }

    public T encontraMaiorElemento() {
        if (this.vazia()) {
            return null;
        }
        if (this.getRaiz().getDir() == null) {
            return this.getRaiz().getInfo();
        }
        return (((NoArvoreBST<T>) this.getRaiz()).encontraMaiorElemento()).getInfo();
    }

    public NoArvoreBST<T> encontraAntecessor(NoArvoreBST<T> no) {
        if (no.getEsq() != null) {
            return ((NoArvoreBST<T>) no.getEsq()).encontraAntecessor();
        }
        return null;
